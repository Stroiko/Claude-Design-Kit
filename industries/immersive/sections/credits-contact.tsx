 ...
 *           nothing loops except the atmosphere), closing on a huge Unbounded contact CTA
 ...
 *       ../DIRECTION.md. The CTA reveal is composed from split-text-title (mode "once");
 *       reduced motion renders the finished title immediately. The CTA is the page's
 *       primary-CTA magenta role (DIRECTION.md) — the credits themselves stay bone/muted.
 */
"use client"
  ...
  return (
    <section className={cn("bg-background py-24 md:py-40", className)}>
      ...
      <div className="mx-auto max-w-2xl px-6 text-center md:px-12">
        ...
        <dl className="mt-14 space-y-10">
          {credits.map((credit) => (
            <div key={`${credit.role}-${credit.name}`}>
              <dt className="font-[Sora] text-xs font-semibold tracking-[0.2em] text-muted-foreground uppercase">
                {credit.role}
              </dt>
              <dd className="mt-2 font-[Sora] text-lg font-semibold text-foreground md:text-xl">
                {credit.name}
              </dd>
            </div>
          ))}
        ...
        {/* The contact CTA — the page's primary-CTA magenta role. A real link, huge type. */}
        <div className="mt-24 md:mt-32">
          ...
          <p className="mt-8 font-[Sora] text-xs font-semibold tracking-[0.2em] text-muted-foreground uppercase">
